import 'package:flutter/material.dart';

final Color accentColor = Color(0xFF202025);
final Color accentLightColor = Color(0xFFdaaa7b);

final Color primaryColor = Color(0xFFa1a1a1);
final Color primaryColorDark = Color(0xFF393838);

final Color facebookColor = Color(0xFFf4d6cac);

final Color backgroundColor = Colors.white;
final Color backgroundDarkColor = Color(0xFF3a3b35);
final Color cardColor = Color(0xFF2b2d34);

final Color backgroundGradient1 = Color(0xFF202025);
final Color backgroundGradient2 = Color(0xFF393838);
final Color backgroundGradient3 = Color(0xFF202025);

final Color disabledTextColor = Color(0xFFa1a1a1);
final Color disabledColor = Colors.black38;
final Color disabledListViewColor = Color(0xFFf7f7f7);

final Color canvasColor = Colors.transparent;

final Color snackbarBackgroundColor = Colors.black87;
final Color toastBackgroundColor = Colors.black87;
final Color toastTextColor = Colors.white;

final Color malibu = Color.fromRGBO(116, 185, 255, 1);
final Color geyser = Color.fromRGBO(223, 230, 233, 1);
final Color zest = Color.fromRGBO(230, 126, 34, 1);
final Color salmon = Color.fromRGBO(255, 118, 117, 1);
